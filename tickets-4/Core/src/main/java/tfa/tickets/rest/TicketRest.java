...
class TicketRest implements ITicketRest
{
  ...
  public Response create(final Ticket t)
  {
    ...
    if (tc == null || tc.getId() == null) throw new RuntimeException("ticket not created");

    // normal status or updated returned
    Status status = Response.Status.CREATED;
    if (t.getId() != null && t.getId() == tc.getId()) status = Response.Status.ACCEPTED;
  ...
  public Response update(final Ticket t)
  {
    ...
    if (t.getId() == null) status = Response.Status.ACCEPTED;
  ...
  public List<Ticket> list(final Integer page, final Integer size, final String order, final String id,
      final String title, final String desc, final String status, final String user, final String date)
  {
    // check asked criteria
    if (page < 0 || size < 0) throw new IllegalArgumentException("negative size or page");

    // find list according these criteria
    List<Ticket> result =
      dao.getList(makeFilter(id, title, desc, status, user, date), makeOrder(order), page.intValue(), size.intValue());
    if (result == null) throw new RuntimeException("null list result");

    return result;
  }

  public Long count(final String id, final String title, final String desc, final String status,
      final String user, final String date)
  {
    // compute count according these criteria
    Long count = dao.count(makeFilter(id, title, desc, status, user, date));
    if (count == null || count < 0L) throw new RuntimeException("no or negative count");
  ...
  private Map<String, Object> makeFilter(final String id, final String title, final String desc, final String status,
      final String user, final String date)
  {
    ...
    if (id != null)
    {
      ...
      String[] ir = id.split("-");
      if (ir.length > 2) throw new IllegalArgumentException();
    ...
    if (status != null)
    {
      ...
      for (String k : l)
        chk.identValid(k, 32);
      if (l.length > 0) filter.put("status", (l.length == 1 ? l[0] : Arrays.asList(l)));
    }
    ...
    if (date != null)
    {
      ...
      if (datei.length > 2) throw new IllegalArgumentException();
